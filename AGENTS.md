...
- 组件文档必须包含组件介绍、默认示例和 API；介绍需同时写明 `mat-*` 模板标签和 PascalCase 组件导出名，方便使用者识别按需导入名称。
- 全局注册与按需导入的实际写法统一在安装文档中维护，不在每个组件页面重复。
- API 按实现情况记录属性、方法、事件、slots 和状态；没有对应公共能力时不创建空章节，也不得把内部 class 或 CSS 自定义属性写成公共接口。
- 属性需说明名称、类型、默认值和用途；方法需说明参数与返回值；事件需说明载荷和触发条件；slot 需说明名称与内容约束；状态需说明用户可观察的行为。
- 新组件至少提供默认样式示例。默认示例之外，只添加用户需求明确涉及且已由公共 API 支持的示例，不预先扩展示例范围。
