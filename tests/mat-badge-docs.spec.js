...
const pagePath = 'docs/site/components/badge.md';
const exampleNames = [
  'BadgeDotExample',
  'BadgeContentExample',
...
describe('Badge 文档', () => {
  it('提供完整组件章节、侧栏入口和 AI 索引', () => {
    ...
    expect(page).toContain('`<mat-badge>` 的组件导出名是 `MatBadge`');
    expect(page).toContain('`location="inline"`');
    ['## 组件简介', '## 示例', '## API', '## 事件', '## Slots'].forEach((heading) => {
      expect(page).toContain(heading);
    });
    expect(config).toContain("{ text: 'Badge 徽标', link: '/components/badge' }");
    expect(llmsGuide).toContain('Badge：`docs/site/components/badge.md`');
  });

  it('每个 Badge 示例都由同一 Vue 文件提供代码和预览', () => {
    const page = readFileSync(resolve(pagePath), 'utf8');

    exampleNames.forEach((exampleName) => {
      const snippetPath = `examples/badge/${exampleName}.vue`;
