...
| `itemHeight` | `number \| string` | `undefined` | 固定的单项高度（单位 px）；仅支持可转换为数字的数值或纯数字字符串。传入时跳过动态尺寸计算与 ResizeObserver 监听。 |
| `estimatedItemHeight` | `number \| string` | `48` | 动态高度模式下的初始预估单项高度（单位 px）。 |
| `buffer` | `number \| string` | `3` | 视口上下方额外预渲染的缓冲项数量。 |
| `itemKey` | `Function \| string` | `undefined` | 用于提取 item 唯一 key 的函数或属性名；未设置时默认使用项的索引 index。 |
| `as` | `string` | `'div'` | 根容器渲染的 HTML 标签名。 |

### 方法

通过模板引用（`ref`）可调用 `MatVirtualScroll` 暴露的公共方法：

| 方法 | 参数 | 返回值 | 说明 |
| --- | --- | --- | --- |
| `scrollToIndex` | `index: number, options?: ScrollToIndexOptions` | `void` | 滚动使指定索引项进入视口。 |
| `scrollTo` | `options: ScrollToOptions` | `void` | 代理调用关联滚动容器的原生 `scrollTo` 方法。 |
| `getScroller` | 无 | `HTMLElement \| Window \| null` | 获取当前关联的滚动容器元素或窗口对象。 |
| `refresh` | 无 | `Promise<void>` | 在 DOM 更新周期后强制重新计算可见区间与占位高度。 |
| `calculate` | 无 | `void` | 立即执行可见区间与占位高度重新计算。 |

`scrollToIndex` 的 `options` 支持以下配置：
- `align`：对齐方式，可选 `'start'`、`'center'`、`'end'` 或 `'auto'`，默认为 `'auto'`；
- `behavior`：滚动动画行为，可选 `'auto'` 或 `'smooth'`，默认为 `'auto'`。

## 事件

| 事件名 | 载荷 | 触发条件 |
| --- | --- | --- |
| `scroll` | `{ scrollTop, scrollHeight, clientHeight, startIndex, endIndex }` | 滚动容器发生滚动时触发。 |
| `visible-range-change` | `{ startIndex, endIndex }` | 渲染的项目索引区间变化时触发。 |

## Slots

| 名称 | 参数 | 说明 |
| --- | --- | --- |
| 默认 | `{ item, index, itemRef }` | 渲染每个可见项的主插槽。动态高度模式下须将 `itemRef` 赋给项根元素。 |
