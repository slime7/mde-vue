...
| `resizable` | `boolean` | `true` | 是否在相邻 Pane 之间显示调整控件 |
...
调整控件在普通、hover、focus-visible、按下和拖动状态之间切换，交互目标为 48px，并在 `MatPanes` 高度方向垂直居中。控件使用 `role="separator"` 与垂直方向语义；左右键每次调整 16px，Shift 加速为 64px，Home/End 移到边界，Enter 在折叠左侧 Pane 与最近一次非零比例间切换。指针拖动期间只更新内部预览，释放后才触发 `update:sizes`。
