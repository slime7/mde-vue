...
`<mat-pull-to-refresh>` 的组件导出名是 `MatPullToRefresh`。它在滚动区域起始端提供 Material 3 风格的下拉刷新手势：垂直 `mat-scroll-area` 滚动到顶部后继续向下拉动、水平 `mat-scroll-area` 滚动到最左端后继续向右拉动，都会随拉动距离渐入内部的 `mat-loading` 指示器；释放时拉动距离达到触发距离即发出刷新，未达到则回弹取消。鼠标、触摸、笔的拖拽与滚轮、触控板在边界朝界外的滚动都能触发。

组件没有 Slots，只负责手势与指示器：把它作为 `mat-scroll-area` 内容的第一个子元素，列表或其他内容作为兄弟元素跟在组件后面，组件渲染为零尺寸元素悬浮在内容上方。开启 `placeholder` 后组件自身随拉动变高（水平时变宽），把后面的内容推离起点，呈现列表"被拉动"的视觉效果。指示器与收起动画的阈值、拖拽系数和弹簧参数对应 AndroidX Material 3 `PullToRefresh` 的官方数值，刷新期间指示器切换到 `mat-loading` 的自动旋转模式。
...
触发刷新时组件把 `modelValue` 置为 `true` 并发出 `refresh`；异步刷新完成后把 `modelValue` 置回 `false`，指示器回弹收起。刷新过程中继续拉动不会重复触发。
...
开启后组件根元素随拉动变高，把后面的兄弟内容推离起点；刷新期间保持足够指示器停留的高度，刷新结束回弹归零。未开启时指示器悬浮在内容上方，内容不移动。
...
| `update:modelValue` | `boolean` | 触发刷新时载荷为 `true`；外部置回 `false` 结束刷新并收起指示器 |
...
拖动过程中内部 `mat-loading` 进入受控进度模式：进度为拉动距离与触发距离之比，`0` 至 `1` 控制圆形到 `soft-burst` 的形变，超过 `1` 后保持 `soft-burst` 并继续旋转；指示器透明度按官方规范从 `0.3` 渐入到 `1`。释放达到阈值后指示器以轻微放大动画停在距起点 `triggerDistance` 处（默认 80px），并切换到 `mat-loading` 的自动旋转模式；未达阈值或刷新结束时按弹簧动画收起。启用减少动态效果偏好时，收起与放大动画直接落位。组件没有公开方法。

## 参考来源

触发阈值（80dp）、拖拽系数（0.5）、指示器行程上限、拖动进度驱动与刷新弹簧（damping 1.0、stiffness 1600）参考 [AndroidX PullToRefresh.kt](https://android.googlesource.com/platform/frameworks/support/+/androidx-main/compose/material3/material3/src/commonMain/kotlin/androidx/compose/material3/pulltorefresh/PullToRefresh.kt)；指示器容器尺寸、出现动画与自动旋转参考 [AndroidX LoadingIndicator.kt](https://android.googlesource.com/platform/frameworks/support/+/androidx-main/compose/material3/material3/src/commonMain/kotlin/androidx/compose/material3/LoadingIndicator.kt)；指示器透明度下限参考 SwipeRefreshLayout 规范。
