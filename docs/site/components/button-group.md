...
按下 standard 子项时，该项宽度临时增大，直接相邻项同步缩窄；connected 只改变当前按钮的按压形状，不联动相邻宽度。standard 的选中按钮会把 `round` 与 `square` 反转；connected 的选中按钮四角都使用 round checked shape，`shape` 只改变未选中组的外部轮廓。组的 `size`、`shape`、`color` 只作为未显式设置子项的默认值，组 `disabled` 与子项 `disabled` 取或。
