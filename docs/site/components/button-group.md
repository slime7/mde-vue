...
按下 standard 子项时，该项宽度临时增大。触发按钮、相邻按钮宽度和水平内边距由同一个无过冲帧进度同步回弹；每帧的子像素余量会合并到最后一个受影响按钮，使组的总宽度与后方按钮位置保持稳定。文字按钮的直接相邻项只压缩水平内边距，文字和图标内容区域保持不变；相邻项可用内边距不足时会限制增宽幅度。纯图标按钮没有水平内边距，组会改为有限压缩相邻图标容器，从而保留弹性反馈。connected 只改变当前按钮的按压形状，不联动相邻宽度。standard 的选中按钮会把 `round` 与 `square` 反转；connected 的选中按钮四角都使用 round checked shape，`shape` 只改变未选中组的外部轮廓。组的 `size`、`shape`、`color` 只作为未显式设置子项的默认值，组 `disabled` 与子项 `disabled` 取或。
