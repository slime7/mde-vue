...
环形路径为所有 shape 和 thickness 共用稳定半径：`size / 2 - 1.6px - heavy 粗细 / 2`。因此 48px 环形的路径半径为 20px；default 不会填满外框，`wavy + heavy` 的最外缘正好到达外框。切换 shape 或 thickness 时，外框和路径半径保持不变。

波浪线的振幅为 3px、波长为 40px；波浪环的径向振幅为 1.6px、波长约为 15px，这些值和 4px 轨道间隙不会随 `size` 缩放。进度为 0 时，线条和环形的 SVG 活动线段通过圆形 `stroke-linecap` 直接形成与线条同粗的起点圆点；有长度的活动段以及轨道断开处同样使用圆口，组件不会在波浪端点上叠加独立圆点。
