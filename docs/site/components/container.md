...
`<mat-container>` 的组件导出名是 `MatContainer`。它的外层始终铺满父容器，并根据浏览器视口在窄于 600px 时使用 16px、其他宽度使用 24px 的水平内边距。位于 `mat-scroll-area` 内时，右侧内边距自动减去滚动条宽度，正文在滚动条占据右侧空间后仍保持视觉居中。默认 Slot 位于正文区域；正文默认最大宽度为 1040px，空间更宽时自动居中。外层具有确定高度时，正文区域同步铺满高度；未确定高度时仍由内容自然撑开。
