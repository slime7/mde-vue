...
基础 Dialog 的图标、标题、正文和 actions 各自提供容器留白，正文与标题之间保持 16px 间距；根容器不裁剪 Slot 内容或焦点环。Dialog 打开期间锁定页面根滚动，最后一层关闭后恢复原有内联样式。标题和动作区固定，正文过长时只有 content 区域沿纵向滚动，滚动条位于 Dialog 内部边缘并预留稳定空间。未设置 `width` 时基础 Dialog 宽度限制为 280–560px；设置后使用指定首选宽度，但小屏仍限制在视口可用范围内。全屏 Dialog 占满视口并使用固定 56px 头部，忽略 `width`。
