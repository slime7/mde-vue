...
`<mat-side-sheet>` 的组件导出名是 `MatSideSheet`。它从视口或布局容器的逻辑侧边展示补充内容，并提供 Material 3 的 `standard` 与 `modal` 两种变体。Standard 适合宽屏 supporting pane，与主内容并排；modal 从侧边覆盖进入并阻止背景交互。位于 `MatAppRoot` 内且省略 `attach` 时，modal 自动进入该 AppRoot 的模态层，表面与帷幕限制在应用矩形内，AppRoot 外内容保持可交互。
...
组件是受控的。Modal 打开后聚焦显式 `autofocus` 或第一个可交互元素，关闭完成后恢复原焦点。Modal 锁定滚动并阻止背景交互：非 AppRoot 场景锁定页面根滚动，AppRoot 场景锁定正文滚动容器并把正文层设为 `inert`；页面原本存在占据布局宽度的经典滚动条时临时保留其槽位，避免页面宽度跳动，没有经典滚动条时不额外预留空间。Standard 不主动移动焦点，也不锁定页面滚动。

在非 AppRoot 场景下，为防止锁定滚动时页面主内容发生宽度跳动，根节点会启用 `scrollbar-gutter: stable` 保留滚动条槽位。由于浏览器标准将该槽位作为视口排版边界的一部分，右侧模态 Side sheet 会对齐在槽位左侧，表现为面板与屏幕物理右边缘之间存在与原滚动条等宽的占位间隙。若需要侧边面板无缝紧贴屏幕右边缘并获得更自然的模态范围，推荐使用 `<mat-app-root>`（`MatAppRoot`）构建应用骨架，将页面滚动托管在内部正文容器中。
