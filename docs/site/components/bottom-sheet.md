...
组件是受控的：收到 `update:modelValue(false)` 后，使用者必须更新绑定值才会退出。Modal 打开后聚焦显式 `autofocus` 或第一个可交互元素，关闭完成后恢复原焦点。Modal 锁定页面根滚动；页面原本存在占据布局宽度的经典滚动条时临时保留其槽位，避免页面宽度跳动，没有经典滚动条时不额外预留空间。Standard 不主动移动焦点，也不锁定页面滚动。
