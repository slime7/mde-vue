...
description: mat-bottom-sheet 的 standard、modal、自适应布局、拖动关闭、帷幕和焦点行为。
llms: true
order: 101
---
...
Bottom sheet 的宽度不超过 640px；窄屏可占满视口宽度，宽于 640px 的视口在两侧至少保留 56px。宽屏顶部至少保留 56px，窄屏展开状态顶部至少保留 72px。内容过长时只滚动内容区，顶部把手、标题和底部区域保持可见。正文区域使用关闭滚动方向默认内边距的垂直 `MatScrollArea` 和 `thin` 滚动条，保留边缘渐隐与正文留白，并对滚动条侧进行宽度补偿。组件没有公共方法，也不暴露拖动过程中的内部位移。
