...
开启 `virtualExpand` 后，`normal` 状态的面板按 `min(内容高度, 可用高度)` 布局，滚动容器覆盖这段完整内容，再通过向下偏移只露出上半部分，屏幕下方仍然保留内容；偏移量等于 `max(0, 面板高度 − 可用高度 ÷ 2)`，内容不足时偏移为 0，表现与不开启时一致。不开启 `virtualExpand` 时面板高度等于可见高度，内容在面板内部滚动，屏幕下方没有内容。内容区向下滚轮或向上滑动时请求 `max`，已经展开时不会重复请求。

把手的鼠标点击不会切换高度；键盘 Enter/Space 仍然执行可访问的状态操作：`min` 与 `normal` 请求 `max`，standard 的展开档请求 `normal`，modal 的展开档请求关闭。拖动时面板先跟手缩放到 `min`，越过后改为整体下滑，向上最多放大到可用高度；松手按相邻档位中点吸附到 `min`、`normal` 或 `max`，落点低于 32px 或向下快速甩动（位移至少 20px、速度不低于 0.35px/ms）时请求关闭。`full` 与自定义高度继续向上拖动不会切换档位，小幅向下拖动也会先回到当前高度；两者都不会成为拖动结果。开启 `virtualExpand` 时拖动只改变预览偏移，向上最多把整块面板移回屏幕，松手后回到吸附到的档位。
...
| `virtualExpand` | `boolean` | `false` | 模板属性为 `virtual-expand`；normal 状态按 min(内容高度, 可用高度) 布局面板并把下半部分留在屏幕下方，内容区可滚动；内容区向下滚轮或向上滑动时请求 max |
...
| 事件 | 载荷 | 触发条件 |
| --- | --- | --- |
| `update:modelValue` | `boolean` | Escape、允许的帷幕点击，或向下拖动落在关闭分区、快速甩动时请求关闭并发出 `false` |
