...
组件默认在声明位置参与文档布局并粘附到滚动容器顶部。small 与 search 在开始滚动时从 `surface` 连续过渡到 `surface container`；medium flexible 和 large flexible 还会分别在前 48px、56px 滚动距离内折叠到 64px。动画由 CSS `scroll-timeline` 和 `animation-timeline` 驱动，不使用 JavaScript 逐帧计算。不支持这些 CSS 能力的浏览器保持静态展开；减少动态效果时保留展开几何，仅播放表面填色。
...
滚动示例把容器元素作为显式 `scrollTarget`。在最新浏览器中缓慢滚动，可以观察高度、标题、subtitle 和背景色共享同一个滚动进度。
...
桌面搜索输入实时更新 `v-model`；按 Enter 或使用默认 leading 搜索按钮发出 `search`。此示例还使用 trailing Slot 提供清空操作。
...
`class` 与 `style` 传给搜索 `<form role="search">` 容器；其他未消费属性传给内部 `MatInputBase` 的原生 `<input type="search">`。
