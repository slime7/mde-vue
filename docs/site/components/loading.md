...
受控模式使用 AndroidX Determinate LoadingIndicator 的两帧参数：从经过 18° 起始旋转处理的圆形变为 `soft-burst`，`0` 到 `1` 之间的进度直接控制形变与 `-progress × 180°` 旋转；进度超过 `1` 后保持 `soft-burst`，继续追加相同方向的旋转。设置 `progress` 会停止自动形状循环和旋转，移除后恢复自动模式。
