...
组件显式 `color` 优先于父组合组件级联值，父级联值优先于形态默认值。由 prop 生成的局部配色优先于同名组件 CSS 定制变量。中性表面、outline 和 disabled 仍使用项目中性角色，不随局部种子色变化。

三位十六进制、带透明度的十六进制、`rgb()`、颜色名称和任意 CSS 值都不是合法 `color` prop。应用如需完全自定义内部角色，应只使用组件文档明确列出的 CSS 入口。
...
上例中的组件变量用于集中表达角色映射；它们只有在对应组件文档明确列出时才是公共 CSS 定制入口。
