...
`dist/styles.css` 由基础令牌与全部 SFC 样式生成，并保留公开的 `mde.tokens`、`mde.components`、`mde.utilities` 与顶层 `mde-final`。`dist/tailwind.css` 来自独立的 Tailwind 映射，`dist/index.d.ts` 包含根入口的组件、插件、指令和命令式函数声明。`dist/` 必须恰好包含这四个文件，且只能通过 `pnpm build` 更新，并与造成变化的源码和文档放在同一提交中。
...
`mde-vue/styles.css` 公开以下稳定的 CSS 级联层顺序：
...
`mde.tokens` 包含公共 reference/system 令牌及默认主题值，`mde.components` 包含全部组件与公共指令普通样式，`mde.utilities` 包含公共排版 class。`mde-final` 是独立顶层名称，只保护不会阻断运行时更新的少量结构与交互安全不变量；禁止在其中设置 `display`、`visibility`、`opacity`、运行时尺寸、位置或变换。未分层的普通应用 CSS 和普通内联样式自然优先于命名层。四个层名及其顺序属于稳定公共接口；`mde-vue/tailwind.css` 保持独立，只提供映射。

Tailwind CSS v4 使用方必须预先声明 `tailwind-theme`、`tailwind-reset`、`mde`、`tailwind-utilities`、`mde-final` 的完整顶层顺序。Preflight 在组件层前标准化原生元素，utilities 在普通组件层后允许显式覆盖，最终层只维持安全不变量。组件库不承诺任意第三方层序都产生相同外观。
...
`dot` 优先于 `content`；`content=0` 有效，空字符串和 undefined 不显示。覆盖模式无内容时仍保留 Slot 包装，Inline 模式无内容时不生成元素。点型约为 6px，自身边缘与 location 指定的目标边缘重合；内容型保持约 16px 高、四向 4px 内边距并原样扩宽，不替使用方截断超过四字符的内容。内容型覆盖方位以目标中线作为行轴边缘锚点，top/bottom 方向分别让指示器顶边或底边越过目标边界 2px。`color` 遵循统一组件配色约定，默认使用 error 与 on-error。
...
`<mat-loading>` 的导出名是 `MatLoading`，省略 `progress` 时表达短时且不确定的加载过程，始终使用 `role="progressbar"` 且不声明具体进度。活动指示器直接复用 `MatShape` 的 `name`、`size` 与 `color` 属性，固定循环官方 7 个形状，并由 Loading 专用、采样起点经过循环对齐的同拓扑 `polygon()` 关键帧连续变形。组件每 650ms 启动一段参考 AndroidX 参数采样的低阻尼弹簧，同一进度同时驱动轮廓与额外 90° 旋转并产生轻微越过和回稳，另以 4666ms 周期叠加整圈匀速旋转。有限 `progress` 会进入受控模式，停止自动形状和旋转，使用经过 18° 起始旋转的圆形到 `soft-burst` 的同拓扑两帧；`0` 至 `1` 控制轮廓和 `-progress × 180°` 旋转，超过 `1` 保持 `soft-burst` 并继续旋转，根元素提供限制在 0 至 1 的 `aria-valuenow`。活动形状始终按 38 : 48 比例居中限制在容器尺寸内；`containment` 布尔属性开启圆形背景容器并换用同组 on-container 内容色。`size` 只接受数字与纯数字字符串，默认 48 并限制在 24 至 240。`color` 遵循统一组件配色约定，只替换活动形状的强调色族。Loading 没有 Slots、方法或自定义事件。减少动态效果偏好会停止自动模式的形状和旋转动画并保留首个形状，受控模式仍显示当前进度。
...
`<mat-select>` 的导出名是 `MatSelect`，使用 MatTextInputBase 字段外观、可见 combobox 和 MatMenu 组合受控选择。选项只来自 `items`：字符串以自身作为 title 和 value，对象通过 `itemTitle`、`itemValue`、`itemSubtitle` 读取字段，分组使用不允许嵌套的 `{ group, items }`。值只接受基础类型；单选空值是 null，多选空值是数组，多选按 items 顺序显示逗号文本或可移除 input Chips，并用移出焦点及无障碍树的 Checkbox 表示菜单项状态。隐藏原生 select 同步 name、form、required、disabled、multiple、options 和 String(value) 表单值，但可见 combobox 承担全部焦点与键盘交互。Select 是项目根据 Text field、Menu、Chip 和 Checkbox 总结的组合，不声明为 Material 官方 Select 规格。
...
Bottom sheet 最大宽度固定为 640px，使用顶部 extra-large 圆角、level 1 阴影和可选 drag handle；`shadow` 与 `rounded` 默认均为 `true`，关闭时分别移除阴影或顶部圆角。其内置布局只有 drag handle、默认内容区域和 footer；默认内容区域不附加内边距，标题、说明、操作控件和内容间距由使用者通过默认 Slot 自行布局，Bottom sheet 不提供 `closable`、`closeLabel`、`title` 属性，也不转发 `header`、`title`、`actions` Slot。`expanded` 默认值为 `'normal'`，具名档位按高度递增为 min、normal、max、full：`'min'` 固定 64px；`'normal'` 取内容完整高度并以可用高度一半封顶，是默认的中间档；`'max'` 按内容完整高度展开且不超过可用高度；`'full'` 直接使用可用高度（容器高度减去宽屏 56px、窄屏 72px 的顶部安全间距）。数字、纯数字字符串和合法 CSS `block-size` 字符串用于自定义高度，实际渲染高度不低于 64px，非法值回退 normal；normal 与 max 以把手行、内容主体和页脚的实测高度渲染为显式长度，让各档位之间的切换都能插值过渡，测量不到内容高度时回退为按内容自然高度渲染。高度切换、阴影和圆角切换连续过渡；拖动时暂时关闭高度过渡，释放后恢复。拖动档位与几何由内部 `bottom-sheet-drag` 模块统一计算：面板先跟手缩放到 min，越过后改为整体下滑；向上拖动不受当前档位上限约束，先放大到可用高度，再按阻尼继续放大并受一个顶部安全间距封顶，因此档位无法提升时仍有跟手反馈，松手按相邻档位中点吸附回 min、normal 或 max；落点低于 32px，或触控与手写笔向下快速甩动（位移至少 48px、速度不低于 0.5px/ms）时请求关闭，鼠标拖动不参与甩动判定，只能把落点拖进关闭分区；full 与自定义高度保留当前高度作为候选，不会成为拖动结果；拖拽产生的关闭保留当前尺寸与偏移，让退出动画从可见位置继续播放。鼠标点击 drag handle 不切换，Enter/Space 在折叠档请求 `'max'`、standard 的展开档请求 `'normal'`、modal 的展开档请求关闭；`virtualExpand` 在 min 与 normal 档都按 min(内容高度, 可用高度) 布局面板，滚动容器覆盖这段完整内容，只把下半部分留在屏幕下方以露出 64px 或可用高度的一半，两档之间切换时面板高度不变、仅整体位移变化，内容区向下滚轮或向上滑动时请求 `'max'`。`closeOnBack` 只接受按下与抬起都落在帷幕上的点击，从把手开始、松手落到帷幕上的拖拽不再请求关闭。Modal 没有内置标题，必须由使用者提供 `aria-label` 或 `aria-labelledby`。Side sheet 继续使用 start/end 逻辑边缘，默认及最大宽度均为 400px、默认显示关闭入口，并允许触摸用户向依附边缘滑动关闭；Side sheet 的标题、正文、header、actions、footer 和 activator 公共行为保持不变。
