...
Bottom sheet 最大宽度固定为 640px，使用顶部 extra-large 圆角、level 1 阴影和可选 drag handle；`shadow` 与 `rounded` 默认均为 `true`，关闭时分别移除阴影或顶部圆角。其内置布局只有 drag handle、默认内容区域和 footer；默认内容区域不附加内边距，标题、说明、操作控件和内容间距由使用者通过默认 Slot 自行布局，Bottom sheet 不提供 `closable`、`closeLabel`、`title` 属性，也不转发 `header`、`title`、`actions` Slot。`expanded` 默认值为 `'normal'`：normal 按内容自然高度展示，内容超过最大高度时只滚动内容区域；`'full'` 使用扣除顶部安全间距后的最大高度；数字、纯数字字符串和合法 CSS `block-size` 字符串用于自定义高度，实际渲染高度不低于 64px，非法值回退 normal。高度切换、阴影和圆角切换连续过渡；拖动时暂时关闭高度过渡，释放后恢复。normal 状态向上拖动请求 `'full'`，向下拖动达到阈值请求关闭；full 或自定义高度向下拖动请求 `'normal'`。鼠标点击 drag handle 不切换，Enter/Space 保留键盘切换；`virtualExpand` 在 normal 状态保留 full 的面板高度并向下偏移，只显示底部预览区域，内容滚动条按 full 高度计算，内容区向下滚轮或向上滑动时请求 `'full'`。Modal 没有内置标题，必须由使用者提供 `aria-label` 或 `aria-labelledby`。Side sheet 继续使用 start/end 逻辑边缘，默认及最大宽度均为 400px、默认显示关闭入口，并允许触摸用户向依附边缘滑动关闭；Side sheet 的标题、正文、header、actions、footer 和 activator 公共行为保持不变。
