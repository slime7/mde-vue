...
交互性能与动效验收以实施时 Chrome 当前稳定版和前一稳定版为基线，但这不把组件库的运行边界改为 Chrome 专用。
...
文档开发服务通过 Vite alias 直接加载 `src/index.js` 与源码样式，修改组件后无需先运行 `pnpm build`；`dist/` 仍由构建和公开入口测试单独验证。
...
示例默认自适应所在窗口高度，不在示例代码中写固定像素高度。需要模拟应用窗口或滚动视口的示例，舞台容器使用 `block-size: 100%` 撑满，并在对应 `DocsPreview` 上通过 `height`（固定高度）或 `max-height`（达到前自适应）控制文档中的预览高度；同一示例在演练场中则撑满预览区。元素自身尺寸（分隔线、列表项密度、拖拽把手等）和模拟真实页面的特殊示例（如 fixed 定位加视口单位）可以保留像素高度。
