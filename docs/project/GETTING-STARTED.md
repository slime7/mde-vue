...
不建议按文件扩展名一律豁免。依赖锁定、脚本编排或工具配置通常没有稳定的行为断言，不应强行添加低价值测试；构建脚本可使用构建命令、产物检查和现有配置检查验证。若基础设置实际改变运行行为，或用户明确要求测试，则恢复相应测试要求。纯说明文字、错别字或无法自动验证的视觉判断也不强制制造失败测试，但仍应运行相关文档检查；组件文档结构、示例同源关系、生成文件和可构建性等已有适当检查的内容，按其检查要求验证。
