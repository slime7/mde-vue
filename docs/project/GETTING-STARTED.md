...
先写测试，再改代码。新增能力或修复缺陷时按以下顺序工作：

1. 添加或更新能够表达目标行为、边界和回归条件的测试。
2. 运行该测试，并确认它因目标能力尚未实现而失败，而不是因语法、环境或测试自身错误而失败。
3. 修改最小范围的实现，使新增测试通过。
4. 运行受影响模块的测试、静态检查和构建检查，确认没有引入回归。

纯说明文字、错别字或无法自动验证的视觉判断不强制制造失败测试；但组件文档结构、示例同源关系、生成文件和可构建性等可以自动验证的内容仍应先补测试。
