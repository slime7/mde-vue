...
Loading 以单个组件的活动指示器表达 Material 3 Expressive 的短时不确定加载形态。活动指示器固定循环官方 7 个形状序列，通过 `clip-path: shape()` 在相邻形状采样帧之间连续变形并叠加整圈缓慢旋转；形状数据由共享的 MatShape 几何表按等弧长采样生成。`containment` 布尔属性控制是否显示圆形背景容器，开启后活动形状按 38:48 比例缩小并换用同组 on-container 内容色；`size` 只接受数字与纯数字字符串并被限制在 24 至 240。根元素始终提供 progressbar ARIA 角色。
