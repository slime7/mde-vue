...
Loading 以单个组件的活动指示器表达 Material 3 Expressive 的短时不确定加载形态。活动指示器直接复用 `MatShape`，固定循环官方 7 个形状序列，通过公开的 `name`、`size` 与 `color` 属性渲染；Loading 另行从共享 `shape()` 路径预生成相同拓扑的 `polygon()` 帧，并对齐整个循环的采样起点以减少轮廓特征滑动。每 650ms 启动一段参考 AndroidX 参数采样的低阻尼弹簧，同一弹簧进度直接动画 `clip-path` 并推进额外 90° 旋转，再叠加 4666ms 的整圈匀速旋转；额外旋转以四个完整形状循环为 CSS 周期，保证周期边界方向连续。活动形状始终按 38:48 比例居中限制在容器尺寸内；`containment` 布尔属性控制是否显示圆形背景容器，开启后换用同组 on-container 内容色；`size` 只接受数字与纯数字字符串并被限制在 24 至 240。根元素始终提供 progressbar ARIA 角色。
