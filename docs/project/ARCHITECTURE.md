...
Navigation 以 `MatNavigationRail` 与 `MatNavigationRailItem` 组合纵向 Expressive navigation rail 和横向 Flexible navigation bar。两种模式默认在声明位置参与父容器布局；Navigation rail 与 drawer 位于最近 `MatLayout` 或 `MatAppRoot` 时统一登记应用边缘并避让正文，不公开 `app` 开关。显式 `mode="fixed"` 时仍可通过 `attach` 使用 Aside 的挂载能力。AppRoot 只向它们提供覆盖层、Snackbar、布局尺寸和边缘上下文等应用能力；modal 展开只保留 collapsed host 占位。`placeholder` 与 `bottomPlaceholder` 按对应布局能力生效。纵向模式提供 collapsed/expanded、可配置展开宽度、standard/modal、可隐藏 expanded 容器，以及覆盖整栏的 thin ScrollArea；Header/FAB/end 在同一滚动层中 sticky 固定，默认内容滚动。展开态默认 Slot 可组合任意直接子内容，收缩态和横向模式只显示直接子级 Item。`alignment` 在纵向剩余高度或横向可用宽度内对齐默认内容，`fullWidth` 通过父级上下文统一控制全部 Item 指示器。父组件通过 provide/inject 统一受控单选、展示用展开状态、导航方向与 Item 布局，Item 保留原生按钮或链接语义及展开过渡；窗口断点切换由使用方负责。
...
Progress 以单个组件的 `linear` 与 `circular` variant 表达两种 Progress indicator 形态。确定状态在根元素上提供 progressbar ARIA 值，不确定状态仅保留进度语义和动画；波浪形活动指示器由内联 SVG 绘制，轨道保持平直。环形宽高由支持数字与纯数字字符串并限制在 24 至 240 的 `size` 控制，路径半径为 heavy 波浪形预留稳定边界；线条形忽略 `size`。两种形态通过 `default` 与 `heavy` 粗细档位保持一致的公共接口。组件只读取系统 primary 与 secondary container 颜色角色，显式 `color` 通过共享局部配色模块替换活动与停止指示器色。

Loading 以单个组件的活动指示器表达 Material 3 Expressive 的短时加载形态。省略 `progress` 时，活动指示器直接复用 `MatShape`，固定循环官方 7 个形状序列，通过公开的 `name`、`size` 与 `color` 属性渲染；Loading 另行从共享 `shape()` 路径预生成相同拓扑的 `polygon()` 帧，并对齐整个循环的采样起点以减少轮廓特征滑动。每 650ms 启动一段参考 AndroidX 参数采样的低阻尼弹簧，同一弹簧进度直接动画 `clip-path` 并推进额外 90° 旋转，再叠加 4666ms 的整圈匀速旋转；额外旋转以四个完整形状循环为 CSS 周期，保证周期边界方向连续。设置有限 `progress` 后停止这组自动帧循环，改用同 96 点拓扑的圆形与 `soft-burst` 两帧确定动画，以经过 18° 起始旋转的圆形到 `soft-burst` 的线性进度取样轮廓，并按 `-progress × 180°` 直接旋转；进度超过 1 时只钳制形变帧，旋转继续累加。活动形状始终按 38:48 比例居中限制在容器尺寸内；`containment` 布尔属性控制是否显示圆形背景容器，开启后换用同组 on-container 内容色；`size` 只接受数字与纯数字字符串并被限制在 24 至 240。根元素始终提供 progressbar ARIA 角色，受控模式额外提供限制在 0 至 1 的 `aria-valuenow`。
