...
Tooltip 的模块级协调器继续保证同一时间只有一个活动实例，并使用以分组容器为键的 WeakMap 保存当前实际显示的实例标识。分组由展示元素最近的 `data-mat-tooltip-group` 祖先明确声明，不根据标签名或组件层级推断；同组内已有实例显示时切换立即显示，未分组时各实例独立按打开延迟打开。省略显式 attach 时，已打开的 top-layer 祖先优先，其次使用目标所属 AppRoot 并读取布局 padding 生成避让矩形，目标不属于当前 AppRoot 时回退 body 与 Toolbar 几何注册表。Plain 与 Rich tooltip 复用同一触发、定位、延迟和堆叠协议；Rich 由显式 rich 或 subhead/action 内容启用，subhead 与 supporting content 同时支持 prop 和 Slot，action 只使用 Slot，指针或焦点进入 Rich 表面时继续维持自动展示。
